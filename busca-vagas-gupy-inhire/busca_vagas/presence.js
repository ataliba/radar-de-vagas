...
const solides = JSON.parse(fs.readFileSync(path.join(DIR, 'solides_presence.json'), 'utf8'));

const map = new Map();
function get(emp){ if(!map.has(emp)) map.set(emp,{empresa:emp,gupy:'',gupy_url:'',inhire:'',inhire_url:'',inhire_vagas_total:'',solides:'',solides_vagas:''}); return map.get(emp); }
...
for (const s of solides){ const e=get(s.empresa); e.solides='Sim'; e.solides_vagas=s.vagas_no_pool; }
...
console.log(`Presença combinada: ${arr.length} empresas (Gupy=${gupy.length}, InHire=${inhire.length}, Solides=${solides.length}, em ambas Gupy+InHire=${both})`);
